#' Conflicts between the tidymicrobiome and other packages
#'
#' This function lists all the conflicts between packages in the tidymicrobiome
...
#' @export
#' @examples
#' tidymicrobiome_conflicts()
tidymicrobiome_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", tidymicrobiome_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "tidymicrobiome_conflicts")
}

tidymicrobiome_conflict_message <- function(x) {
  if (length(x) == 0)
    return("")
  
  header <- cli::rule(left = crayon::bold("Conflicts"),
                      right = "tidymicrobiome_conflicts()")
...
#' @export
print.tidymicrobiome_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(tidymicrobiome_conflict_message(x))
}
